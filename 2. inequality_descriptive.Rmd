...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'Inequality Descriptive.html'))})
---
...
#### Data from 2021 as the reference (2021 has 194 countries)
```{r tidy=TRUE}
gdp_2021 <- gdp_data %>% filter(Year == 2021)
gdp_21_code <- gdp_2021 %>% select(1)
```

#### Data from 2000 to 2021 (2013 to 2021 has 194 countries)
```{r tidy=TRUE}
dat_gdp <- inner_join(gdp_21_code, gdp_data)
dat_gdp <- dat_gdp[order(dat_gdp$Country, dat_gdp$Year),]
```

#### Mean Per Capita GDP and Per capita gdp Gini index by Year 2013-2021 (same 194 countries)
```{r tidy=TRUE}
...
#### Per Capita CONSUMPTIOM Expenditure (144 countries from 2015 to 2021)
```{r tidy=TRUE}
...
#### Mean GDP and Consumption Expenditure for 144 nations
```{r tidy=TRUE}
...
#### Per capita gdp Gini index by Year (2015-2021 144 countries, 136 for 2022)
```{r tidy=TRUE}
...
#### 144 countries with both GDP and Consumption Expenditure Data (dat_all data)
```{r tidy=TRUE}
...
total_pop144 <- summarize(gdp_con144_2021, total_pop = sum(pop), n())
```

#### GDP and Consumption data 2019, 2020 and 2021 144 common nations
```{r tidy=TRUE}
...
gdp_con44_2020 <- gdp_con44 %>% filter(Year==2020)
...
#### mean GDP and consumption plot for 144 countries
```{r tidy=TRUE}
...
#### Mean and Histogram in one plot
...
#### Cumulative GDP values calculations 
```{r tidy=TRUE}
## 2019
...
#### weighted and Unweighted Gini (Income and consumption) for 144 countries 
```{r tidy=TRUE}
...
#### Lorenz Asymmetry Coefficient Calculations: if LAC < 1, the point where the Lorenz curve is parallel to the line of equality is below the axis of symmetry and the inequality is caused by relatively many poor nations. 
...
dev.off()
```
#### Weighted GINI with and without India, China or US (2016-2021)
```{r tidy=TRUE}
...
#### Consumption GINI with or without India, China, or US
```{r tidy=TRUE}
...
wg_con41 <- ggplot(wgdpcon_gini, aes(x = Year))+
  geom_line(aes(y = wg_con, color = "All"), linewidth = 1.0)+
...
#### Weighted LAC with and without India, China or US
```{r tidy=TRUE}
nchina <- gdp_con44 %>% filter(Country !="China")
nindia <- gdp_con44 %>% filter(Country != "India")
nUS <- gdp_con44 %>% filter(Country !="United States")
...
mld44_plot <- ggplot(mld44, aes(x = Year)) +
  ...
  theme(text=element_text(size=8))+
  scale_x_continuous(breaks = seq(min(av_gdpcon44$Year), max(av_gdpcon44$Year), 1)) +
  scale_y_continuous(limits = c(0.32, 0.64), breaks = seq(0.32, 0.64, 0.04)) +
  theme(axis.text.x = element_text(angle = 90, vjust=0.5))+
  theme(legend.box = "horizontal") +
  theme(legend.direction = "vertical") +
  theme(legend.position = c(0.56, 0.9)) +
  theme(legend.background = element_rect(fill = "transparent")) +
...
#### MLD with or without India, China, or US
```{r tidy=TRUE}
mldgc144 <- mldgc44
mldgc144$`ncon`<- NULL
...
```{r fig.height = 3.5, fig.width = 7, fig.align = "center"}
mld_plot <-ggarrange(mld44_plot, mld_gc44, labels = c("a", "b"), hjust = -5, 
                     font.label = list(size = 8, color = "black", face = "plain"), 
                     ncol = 2, nrow = 1)

file.plot <- file.path(dir.plots, "mld_plot.png")
png(file.plot, height=3.5, width=7, units="in", bg="white", res=300)
mld_plot
dev.off()
```

#### per capita GDP and Consumption of US,India and China
```{r tidy=TRUE}
...
#### 2019 and 2020 for selected countries
```{r tidy=TRUE}
gdp_con2019 <- gdp_con44_2019 %>% select(Code,Country, pop, in_class, Year, pc_gdp, pcon)
gdp_con2020 <-gdp_con44_2020 %>% select(Code, Country, pop, in_class, Year, pc_gdp, pcon)
gdp_condata <- merge(gdp_con2019, gdp_con2020, by = "Code", suffixes = c("_2019", "_2020"))

# Calculate the GDP growth rate
gdp_condata$gdp_growth <- ((gdp_condata$pc_gdp_2020 - gdp_condata$pc_gdp_2019) / gdp_condata$pc_gdp_2019) * 100
gdp_condata$con_growth <- ((gdp_condata$pcon_2020 - gdp_condata$pcon_2019) / gdp_condata$pcon_2019) * 100
gdp_condata[order(gdp_condata$pc_gdp_2019),]
```
...
gdp_condata$ccode <- gdp_condata$Code %in% c("TLS", "COD", "CPV", "ETH", "BRA", "IDN", "PAK", "IND", "CHN", 
                                            "MEX", "JPN", "FRA", "GBR", "RUS", "BHS", "IRL", "USA", "MAC")

gdp_growthplot <- ggplot(gdp_condata, aes(x = reorder(Code, pc_gdp_2019), y=gdp_growth, color=in_class_2019)) + 
  geom_point(size=1)+
  geom_hline(data = ggrowth_mean, aes(yintercept= -3.84), linetype="dotted", linewidth = 0.5)+
  theme_minimal() +
  labs(x = "", y = "Per capita GDP growth rate (%)")+
  theme(axis.title.y=element_text(size=9), axis.text.x=element_blank())+
  theme(text=element_text(size=8))+
  geom_text_repel(data = subset(gdp_condata, ccode),
                 mapping = aes(x= Code, y=gdp_growth, label = Code), color = "black", size = 2)+
  theme(legend.title = element_blank()) +
  theme(legend.direction = "horizontal") +
  ...
  scale_color_discrete(breaks=c('H',"UM", "LM",'L'), labels=c('High Income', "Upper Middle Income", 'Lower Middle Income', "Low Income"))
gdp_growthplot
```


#### Consumption growth rate (2019 to 2020)
```{r tidy=TRUE}

gdp_condata$concode <- gdp_condata$Code %in% c("TLS", "DJI", "GNB", "COD", "BLZ", "PSE", "ETH", "BRA", "PER", "IDN", "MUS", "GEO", "CHN", 
                                            "MEX", "OMN", "JPN", "FRA", "GBR", "RUS", "HKG", "BHS", "IRL", "USA", "MAC")

con_growthplot <- ggplot(gdp_condata, aes(x = reorder(Code, pc_gdp_2019), y=con_growth, color=in_class_2019)) + 
  geom_point(size=1)+
  geom_hline(data = cgrowth_mean, aes(yintercept= -5.19), linetype="dotted", linewidth = 0.5)+
  theme_minimal() +
  theme(axis.title.y = element_text(size =9),legend.position = "none")+
  labs(x = "", y = "Per capita consumption growth rate (%)")+
  theme(text=element_text(size=8))+
  theme(axis.text.x=element_blank())+
  scale_y_continuous(breaks = seq(-30, 15, 5), limits = c(-30, 15)) +
  geom_text_repel(data = subset(gdp_condata, concode),
                 mapping = aes(x= Code, y=con_growth, label = Code), color = "blue3", size = 2)+
  theme(legend.title = element_blank()) +
  theme(legend.direction = "horizontal") +
  theme(legend.position = "none") +
  theme(legend.box = "horizontal")+
...
#### Plot gdp and consumption grwoth plots together

```{r fig.height = 3.5, fig.width = 7, fig.align = "center"}
gdpcon_growth_plot <-ggarrange(gdp_growthplot, con_growthplot, labels = c("a", "b"), hjust = -5, 
                     font.label = list(size = 8, color = "black", face = "plain"), 
                     ncol = 2, nrow = 1, common.legend = TRUE, legend = "bottom")
gdpcon_growth_plot <- annotate_figure(gdpcon_growth_plot, bottom = text_grob("Countries (ranked by 2019 GDP per capita)", 
                      vjust= -5, size =9))


file.plot <- file.path(dir.plots, "gdpcon_growth_plot.png")
png(file.plot, height=3.5, width=7, units="in", bg="white", res=300)
gdpcon_growth_plot
dev.off()
```